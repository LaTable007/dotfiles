-- Table générée depuis les mappings de kvndrsslr/sketchybar-app-font (CC0),
-- qui associent un nom d'application au glyphe correspondant de la police
-- sketchybar-app-font.
--
-- Les noms sont ceux que macOS affiche, donc localisés : « Musique » et non
-- « Music », « Aperçu » et non « Preview ». C'est bien ce nom affiché
-- qu'AeroSpace transmet. La table amont couvre déjà une partie des traductions
-- françaises ; celles qui manquaient sont regroupées à la fin du fichier.
--
-- Pour la régénérer :
--   git clone --depth 1 https://github.com/kvndrsslr/sketchybar-app-font
--   puis convertir mappings/ en paires nom -> glyphe.

local icon_map_table = {
  ["Live"] = ":ableton:",
  ["Acrobat"] = ":acrobat:",
  ["Activity Monitor"] = ":activity_monitor:",
  ["Aktivitätsanzeige"] = ":activity_monitor:",
  ["Монітор активності"] = ":activity_monitor:",
  ["Монитор активности"] = ":activity_monitor:",
  ["aw-tauri"] = ":activitywatch:",
  ["Actual"] = ":actual:",
  ["AdGuard VPN"] = ":adguard_vpn:",
  ["Adobe Bridge"] = ":adobe_bridge:",
  ["AFFiNE"] = ":affine:",
  ["Affinity"] = ":affinity:",
  ["Affinity Designer"] = ":affinity_designer:",
  ["Affinity Designer 2"] = ":affinity_designer_2:",
  ["Affinity Photo"] = ":affinity_photo:",
  ["Affinity Photo 2"] = ":affinity_photo_2:",
  ["Affinity Publisher"] = ":affinity_publisher:",
  ["Affinity Publisher 2"] = ":affinity_publisher_2:",
  ["Airmail"] = ":airmail:",
  ["AirPort Utility"] = ":airport_utility:",
  ["AirVPN"] = ":airvpn:",
  ["Eddie"] = ":airvpn:",
  ["Alacritty"] = ":alacritty:",
  ["Alfred"] = ":alfred:",
  ["AltTab"] = ":alttab:",
  ["AmneziaVPN"] = ":amnezia_vpn:",
  ["BlueStacks"] = ":android:",
  ["HD-MultiInstanceManager"] = ":android:",
  ["Android Messages"] = ":android_messages:",
  ["Android Studio"] = ":android_studio:",
  ["Anki"] = ":anki:",
  ["Antigravity"] = ":antigravity:",
  ["Antigravity IDE"] = ":antigravity:",
  ["AnyDesk"] = ":anydesk:",
  ["Anytype"] = ":anytype:",
  ["ApiArk"] = ":apiark:",
  ["Apifox"] = ":apifox:",
  ["App Eraser"] = ":app_eraser:",
  ["App Store"] = ":app_store:",
  ["Apple Books"] = ":apple_books:",
  ["Books"] = ":apple_books:",
  ["Bücher"] = ":apple_books:",
  ["Apple Configurator"] = ":apple_configurator:",
  ["TV"] = ":apple_tv:",
  ["Arc"] = ":arc:",
  ["Arduino"] = ":arduino:",
  ["Arduino IDE"] = ":arduino:",
  ["Aseprite"] = ":aseprite:",
  ["Atom"] = ":atom:",
  ["Audacity"] = ":audacity:",
  ["Audio MIDI Setup"] = ":audio_midi_setup:",
  ["AusweisApp"] = ":ausweisapp:",
  ["Automator"] = ":automator:",
  ["Azahar"] = ":azahar:",
  ["Backblaze"] = ":backblaze:",
  ["Backblaze Downloader"] = ":backblaze:",
  ["Backblaze Restore"] = ":backblaze:",
  ["BaiduNetdisk"] = ":baidunetdisk:",
  ["百度网盘"] = ":baidunetdisk:",
  ["balenaEtcher"] = ":balena_etcher:",
  ["Bambu Studio"] = ":bambu_studio:",
  ["MoneyMoney"] = ":bank:",
  ["Basecamp"] = ":basecamp:",
  ["Basecamp.app"] = ":basecamp:",
  ["Battle.net"] = ":battle_net:",
  ["Bazaar"] = ":bazaar:",
  ["Bazecor"] = ":bazecor:",
  ["BBEdit"] = ":bbedit:",
  ["Bear"] = ":bear:",
  ["Beekeeper Studio"] = ":beekeeper_studio:",
  ["Beeper"] = ":beeper:",
  ["Beeper Desktop"] = ":beeper:",
  ["Betterbird"] = ":betterbird:",
  ["BetterTouchTool"] = ":bettertouchtool:",
  ["Bilibili"] = ":bilibili:",
  ["哔哩哔哩"] = ":bilibili:",
  ["Bitwarden"] = ":bit_warden:",
  ["Bitwig Studio"] = ":bitwig_studio:",
  ["Blender"] = ":blender:",
  ["Blitzit"] = ":blitzit:",
  ["Bluesky"] = ":bluesky:",
  ["Sky"] = ":bluesky:",
  ["Bluetooth File Exchange"] = ":bluetooth:",
  ["Bluetooth"] = ":bluetooth:",
  ["BluOS Controller"] = ":bluos_controller:",
  ["Calibre"] = ":book:",
  ["Книги"] = ":book:",
  ["YABA"] = ":bookmark:",
  ["Anybox"] = ":bookmark:",
  ["URL Manager Pro"] = ":bookmark:",
  ["Brain.fm"] = ":brainfm:",
  ["Brave Browser"] = ":brave_browser:",
  ["Broadcasts"] = ":broadcasts:",
  ["Bruno"] = ":bruno:",
  ["Burn"] = ":burn:",
  ["Burp Suite"] = ":burp_suite:",
  ["BusyCal"] = ":busycal:",
  ["Buzz"] = ":buzz:",
  ["Calculator"] = ":calculator:",
  ["Calculette"] = ":calculator:",
  ["Rechner"] = ":calculator:",
  ["Калькулятор"] = ":calculator:",
  ["Calendar"] = ":calendar:",
  ["日历"] = ":calendar:",
  ["Fantastical"] = ":calendar:",
  ["Cron"] = ":calendar:",
  ["Amie"] = ":calendar:",
  ["Calendrier"] = ":calendar:",
  ["カレンダー"] = ":calendar:",
  ["Notion Calendar"] = ":calendar:",
  ["Kalender"] = ":calendar:",
  ["Календар"] = ":calendar:",
  ["Календарь"] = ":calendar:",
  ["calibre"] = ":calibre:",
  ["Capacities"] = ":capacities:",
  ["Caprine"] = ":caprine:",
  ["ChatGPT Atlas"] = ":chatgpt_atlas:",
  ["Cherry Studio"] = ":cherry_studio:",
  ["Chess"] = ":chess:",
  ["The Enemy Within"] = ":chess:",
  ["Amazon Chime"] = ":chime:",
  ["Choosy"] = ":choosy:",
  ["Cisco AnyConnect Secure Mobility Client"] = ":cisco_anyconnect:",
  ["Cisco Secure Client"] = ":cisco_anyconnect:",
  ["Citra"] = ":citra:",
  ["Citrix Workspace"] = ":citrix:",
  ["Citrix Viewer"] = ":citrix:",
  ["ClassIn"] = ":classin:",
  ["Claude"] = ":claude:",
  ["ClickUp"] = ":click_up:",
  ["CLion"] = ":clion:",
  ["Clock"] = ":clock:",
  ["cmux"] = ":cmux:",
  ["cmux NIGHTLY"] = ":cmux:",
  ["coconutBattery"] = ":coconut_battery:",
  ["Code"] = ":code:",
  ["Code - Insiders"] = ":code:",
  ["Codex"] = ":codex:",
  ["Cold Turkey Blocker"] = ":cold_turkey_blocker:",
  ["Color Picker"] = ":color_picker:",
  ["数码测色计"] = ":color_picker:",
  ["Кольоромір"] = ":color_picker:",
  ["Цифровой колориметр"] = ":color_picker:",
  ["Digital Color Meter"] = ":color_picker:",
  ["Comet"] = ":comet_browser:",
  ["Conductor"] = ":conductor:",
  ["Console"] = ":console:",
  ["Contacts"] = ":contacts:",
  ["Copilot"] = ":copilot:",
  ["CotEditor"] = ":coteditor:",
  ["Craft"] = ":craft:",
  ["Creative Cloud"] = ":creative_cloud:",
  ["CrossOver"] = ":crossover:",
  ["Cubase"] = ":cubase:",
  ["Cubase Pro"] = ":cubase:",
  ["CurseForge"] = ":curseforge:",
  ["Cursor"] = ":cursor:",
  ["Cypress"] = ":cypress:",
  ["DaisyDisk"] = ":daisy_disk:",
  ["Dash"] = ":dash:",
  ["DataGrip"] = ":datagrip:",
  ["DataSpell"] = ":dataspell:",
  ["DaVinci Resolve"] = ":davinciresolve:",
  ["DBeaver"] = ":dbeaver:",
  ["DeepL"] = ":deepl:",
  ["DeepSeek"] = ":deepseek:",
  ["Deezer"] = ":deezer:",
  ["Default"] = ":default:",
  ["Default Folder X"] = ":default_folder_x:",
  ["Delta"] = ":delta:",
  ["Deluge"] = ":deluge:",
  ["deno"] = ":deno:",
  ["CleanMyMac X"] = ":desktop:",
  ["Devin"] = ":devin:",
  ["DEVONthink 3"] = ":devonthink3:",
  ["DEVONthink"] = ":devonthink3:",
  ["Dexcom"] = ":dexcom:",
  ["Dia"] = ":dia:",
  ["Dictionary"] = ":dictionary:",
  ["DingTalk"] = ":dingtalk:",
  ["钉钉"] = ":dingtalk:",
  ["阿里钉"] = ":dingtalk:",
  ["Discord"] = ":discord:",
  ["Discord Canary"] = ":discord:",
  ["Discord PTB"] = ":discord:",
  ["Disk Utility"] = ":disk_utility:",
  ["Docker"] = ":docker:",
  ["Docker Desktop"] = ":docker:",
  ["GrandTotal"] = ":dollar:",
  ["Receipts"] = ":dollar:",
  ["Dolphin"] = ":dolphin:",
  ["Dorico"] = ":dorico:",
  ["Dorico Pro"] = ":dorico:",
  ["Dota 2"] = ":dota2:",
  ["Double Commander"] = ":doublecmd:",
  ["Downie 4"] = ":downie:",
  ["Drafts"] = ":drafts:",
  ["draw.io"] = ":draw_io:",
  ["Dropbox"] = ":dropbox:",
  ["Dropover"] = ":dropover:",
  ["DSM"] = ":dsm:",
  ["DVD Player"] = ":dvd_player:",
  ["EA app"] = ":ea:",
  ["Eagle"] = ":eagle:",
  ["Electron"] = ":electron:",
  ["Element"] = ":element:",
  ["Emacs"] = ":emacs:",
  ["Epic Games Launcher"] = ":epic_games:",
  ["EuDic"] = ":eudic:",
  ["Evernote Legacy"] = ":evernote_legacy:",
  ["Evernote"] = ":evernote_legacy:",
  ["FaceTime"] = ":face_time:",
  ["FaceTime 通话"] = ":face_time:",
  ["Feedback Assistant"] = ":feedback_assistant:",
  ["Feishu"] = ":feishu:",
  ["飞书"] = ":feishu:",
  ["飞书会议"] = ":feishu:",
  ["Figma"] = ":figma:",
  ["FileBot"] = ":filebot:",
  ["FileZilla"] = ":filezilla:",
  ["Final Cut Pro"] = ":final_cut_pro:",
  ["Finamp"] = ":finamp:",
  ["Find My"] = ":find_my:",
  ["Локатор"] = ":find_my:",
  ["Finder"] = ":finder:",
  ["访达"] = ":finder:",
  ["Bloom"] = ":finder:",
  ["FlowFinder"] = ":finder:",
  ["Firefox"] = ":firefox:",
  ["Firefox Developer Edition"] = ":firefox_developer_edition:",
  ["Firefox Nightly"] = ":firefox_developer_edition:",
  ["Flaresolverr"] = ":flaresolverr:",
  ["Floorp"] = ":floorp:",
  ["FlowJo"] = ":flowjo:",
  ["FL Studio"] = ":flstudio:",
  ["Fluxer"] = ":fluxer:",
  ["FMOD Studio"] = ":fmod:",
  ["Folx"] = ":folx:",
  ["Font Book"] = ":font_book:",
  ["foobar2000"] = ":foobar2000:",
  ["Fork"] = ":fork:",
  ["ForkLift"] = ":forklift:",
  ["Foxit PDF Reader"] = ":foxit_reader:",
  ["FreeLLMAPI"] = ":free_llm_api:",
  ["FreeCAD"] = ":freecad:",
  ["Freeform"] = ":freeform:",
  ["Freenet"] = ":freenet:",
  ["FreeTube"] = ":freetube:",
  ["FSNotes"] = ":fsnotes:",
  ["Fusion"] = ":fusion:",
  ["Games"] = ":games:",
  ["System Preferences"] = ":gear:",
  ["System Settings"] = ":gear:",
  ["系统设置"] = ":gear:",
  ["Réglages Système"] = ":gear:",
  ["システム設定"] = ":gear:",
  ["Systemeinstellungen"] = ":gear:",
  ["System­einstellungen"] = ":gear:",
  ["Системні параметри"] = ":gear:",
  ["Системные настройки"] = ":gear:",
  ["Gemini"] = ":gemini:",
  ["Google Gemini"] = ":gemini:",
  ["Ghidra"] = ":ghidra:",
  ["Ghostty"] = ":ghostty:",
  ["GIMP"] = ":gimp:",
  ["GitHub Desktop"] = ":git_hub:",
  ["GitHub Copilot"] = ":git_hub:",
  ["GitKraken"] = ":gitkraken:",
  ["Godot"] = ":godot:",
  ["GOG Galaxy"] = ":gog_galaxy:",
  ["GoLand"] = ":goland:",
  ["GoodLinks"] = ":goodlinks:",
  ["Goodnotes"] = ":goodnotes:",
  ["Google Chat"] = ":google_chat:",
  ["Chromium"] = ":google_chrome:",
  ["Google Chrome"] = ":google_chrome:",
  ["Google Chrome Canary"] = ":google_chrome:",
  ["Google Drive"] = ":google_drive:",
  ["Google Earth Pro"] = ":google_earth:",
  ["Google Earth"] = ":google_earth:",
  ["Grammarly Editor"] = ":grammarly:",
  ["Granola"] = ":granola:",
  ["Grapher"] = ":grapher:",
  ["Grayjay"] = ":grayjay:",
  ["Hammerspoon"] = ":hammerspoon:",
  ["HandBrake"] = ":handbrake:",
  ["Hazel"] = ":hazel:",
  ["HBO Max"] = ":hbo_max:",
  ["Helium"] = ":helium:",
  ["Hermes"] = ":hermes_agent:",
  ["Hermes Agent"] = ":hermes_agent:",
  ["Hermes Agent Desktop"] = ":hermes_agent:",
  ["Hex Fiend"] = ":hex_fiend:",
  ["Home"] = ":home:",
  ["Home Assistant"] = ":home_assistant:",
  ["brew-browser"] = ":homebrew:",
  ["WailBrew"] = ":homebrew:",
  ["TapHouse"] = ":homebrew:",
  ["Cork"] = ":homebrew:",
  ["Homebrew"] = ":homebrew:",
  ["Brewer X"] = ":homebrew:",
  ["Applite"] = ":homebrew:",
  ["BrewMate"] = ":homebrew:",
  ["Brewlet"] = ":homebrew:",
  ["Hyper"] = ":hyper:",
  ["Hyperkey"] = ":hyperkey:",
  ["iA Writer"] = ":ia_writer:",
  ["Ice Cubes"] = ":ice_cubes:",
  ["IntelliJ IDEA"] = ":idea:",
  ["IINA"] = ":iina:",
  ["Adobe Illustrator"] = ":illustrator:",
  ["Illustrator"] = ":illustrator:",
  ["Image Playground"] = ":image_playground:",
  ["ImHex"] = ":imhex:",
  ["Adobe InDesign"] = ":indesign:",
  ["InDesign"] = ":indesign:",
  ["Infuse"] = ":infuse:",
  ["Inkdrop"] = ":inkdrop:",
  ["Inkscape"] = ":inkscape:",
  ["Insomnia"] = ":insomnia:",
  ["Instagram"] = ":instagram:",
  ["Instapaper"] = ":instapaper:",
  ["Instruments"] = ":instruments:",
  ["Invoice Ninja"] = ":invoice_ninja:",
  ["InvoiceNinja"] = ":invoice_ninja:",
  ["invoiceninja"] = ":invoice_ninja:",
  ["iPhone Mirroring"] = ":iphone_mirroring:",
  ["Iris"] = ":iris:",
  ["iTerm"] = ":iterm:",
  ["iTerm2"] = ":iterm:",
  ["Product Portal"] = ":izotope:",
  ["iZotope RX 9"] = ":izotope:",
  ["iZotope RX 10"] = ":izotope:",
  ["iZotope RX 11"] = ":izotope:",
  ["iZotope RX 12"] = ":izotope:",
  ["iZotope Ozone 9"] = ":izotope:",
  ["iZotope Ozone 10"] = ":izotope:",
  ["iZotope Ozone 11"] = ":izotope:",
  ["iZotope Ozone 12"] = ":izotope:",
  ["Jan"] = ":jan:",
  ["Jane Reader"] = ":jane_reader:",
  ["JDownloader"] = ":jdownloader:",
  ["JDownloader2"] = ":jdownloader:",
  ["Uninstall JDownloader"] = ":jdownloader:",
  ["Jellyfin Media Player"] = ":jellyfin:",
  ["Jellyfin Desktop"] = ":jellyfin:",
  ["Jellyseer"] = ":jellyseer:",
  ["JetBrains Gateway"] = ":jetbrains_gateway:",
  ["JetBrains Toolbox"] = ":jetbrains_toolbox:",
  ["Jira"] = ":jira:",
  ["Joplin"] = ":joplin:",
  ["Journal"] = ":journal:",
  ["JuxtaText"] = ":juxtatext:",
  ["카카오톡"] = ":kakaotalk:",
  ["KakaoTalk"] = ":kakaotalk:",
  ["Kakoune"] = ":kakoune:",
  ["Karabiner-Elements Settings"] = ":karabiner_elements:",
  ["Karabiner-Elements"] = ":karabiner_elements:",
  ["Karabiner-EventViewer"] = ":karabiner_elements_event_viewer:",
  ["KeePassXC"] = ":kee_pass_x_c:",
  ["Keyboard Maestro"] = ":keyboard_maestro:",
  ["Keychain Access"] = ":keychain_access:",
  ["Keynote"] = ":keynote:",
  ["Keynote 讲演"] = ":keynote:",
  ["KiCad"] = ":kicad:",
  ["Kiro"] = ":kiro:",
  ["Kiro Crew"] = ":kiro_crew:",
  ["kitty"] = ":kitty:",
  ["KiwiDesk"] = ":kiwidesk:",
  ["Kodi"] = ":kodi:",
  ["krita"] = ":krita:",
  ["LanguageTool for Desktop"] = ":languagetool_for_desktop:",
  ["Latest"] = ":latest:",
  ["League of Legends"] = ":league_of_legends:",
  ["legcord"] = ":legcord:",
  ["Legcord"] = ":legcord:",
  ["LibreWolf"] = ":libre_wolf:",
  ["LibreOffice"] = ":libreoffice:",
  ["Lidarr"] = ":lidarr:",
  ["Adobe Lightroom"] = ":lightroom:",
  ["Lightroom Classic"] = ":lightroomclassic:",
  ["LINE"] = ":line:",
  ["Linear"] = ":linear:",
  ["Little Snitch"] = ":little_snitch:",
  ["Little Snitch Network Monitor"] = ":little_snitch:",
  ["Little Snitch Mini"] = ":little_snitch_mini:",
  ["LM Studio"] = ":lm_studio:",
  ["LobeHub"] = ":lobehub:",
  ["LocalSend"] = ":localsend:",
  ["Logic Pro"] = ":logicpro:",
  ["Logseq"] = ":logseq:",
  ["LuLu"] = ":lulu:",
  ["Maccy"] = ":maccy_clip:",
  ["MacForge"] = ":macforge:",
  ["MacPass"] = ":macpass:",
  ["macshot"] = ":macshot:",
  ["macshot.app"] = ":macshot:",
  ["MacShot"] = ":macshot:",
  ["Mactracker"] = ":mactracker:",
  ["Magnifier"] = ":magnifier:",
  ["Canary Mail"] = ":mail:",
  ["HEY"] = ":mail:",
  ["Mail"] = ":mail:",
  ["Mailspring"] = ":mail:",
  ["MailMate"] = ":mail:",
  ["Superhuman"] = ":mail:",
  ["Spark"] = ":mail:",
  ["Spark Mail"] = ":mail:",
  ["邮件"] = ":mail:",
  ["メール"] = ":mail:",
  ["Пошта"] = ":mail:",
  ["Почта"] = ":mail:",
  ["MakeMKV"] = ":makemkv:",
  ["MAMP"] = ":mamp:",
  ["MAMP PRO"] = ":mamp:",
  ["Maps"] = ":maps:",
  ["Google Maps"] = ":maps:",
  ["マップ"] = ":maps:",
  ["Karten"] = ":maps:",
  ["Карти"] = ":maps:",
  ["Карты"] = ":maps:",
  ["Marked 2"] = ":marked_2:",
  ["MarkText"] = ":marktext:",
  ["MarkText+"] = ":marktext:",
  ["Marta"] = ":marta:",
  ["Matlab"] = ":matlab:",
  ["MATLAB"] = ":matlab:",
  ["MATLABWindow"] = ":matlab:",
  ["MATLAB_R2024b"] = ":matlab:",
  ["MATLAB_R2024a"] = ":matlab:",
  ["MATLAB_R2023b"] = ":matlab:",
  ["MATLAB_R2023a"] = ":matlab:",
  ["MATLAB_R2022b"] = ":matlab:",
  ["MATLAB_R2022a"] = ":matlab:",
  ["MATLAB_R2021b"] = ":matlab:",
  ["MATLAB_R2021a"] = ":matlab:",
  ["Mattermost"] = ":mattermost:",
  ["MediaInfo"] = ":mediainfo:",
  ["Google Meet"] = ":meet:",
  ["MEGA"] = ":mega:",
  ["Messages"] = ":messages:",
  ["信息"] = ":messages:",
  ["Nachrichten"] = ":messages:",
  ["メッセージ"] = ":messages:",
  ["Повідомлення"] = ":messages:",
  ["Сообщения"] = ":messages:",
  ["Messenger"] = ":messenger:",
  ["Microsoft Edge"] = ":microsoft_edge:",
  ["Microsoft Excel"] = ":microsoft_excel:",
  ["Microsoft Outlook"] = ":microsoft_outlook:",
  ["Microsoft PowerPoint"] = ":microsoft_power_point:",
  ["Microsoft Remote Desktop"] = ":microsoft_remote_desktop:",
  ["Microsoft Teams"] = ":microsoft_teams:",
  ["Microsoft Teams (work or school)"] = ":microsoft_teams:",
  ["Microsoft Word"] = ":microsoft_word:",
  ["Migaku"] = ":migaku:",
  ["Mimestream"] = ":mimestream:",
  ["Min"] = ":min_browser:",
  ["Minecraft"] = ":minecraft:",
  ["Minecraft Launcher"] = ":minecraft:",
  ["Miro"] = ":miro:",
  ["Mole"] = ":mole:",
  ["MongoDB Compass"] = ":mongodb:",
  ["Moonlight"] = ":moonlight:",
  ["Motrix"] = ":motrix:",
  ["Movist Pro"] = ":movist_pro:",
  ["Mp3tag"] = ":mp3tag:",
  ["mpv"] = ":mpv:",
  ["MQTTX"] = ":mqttx:",
  ["MTGA"] = ":mtga:",
  ["Mullvad Browser"] = ":mullvad_browser:",
  ["Mullvad VPN"] = ":mullvad_vpn:",
  ["MuseHub"] = ":musehub:",
  ["MuseScore"] = ":musescore:",
  ["MuseScore 4"] = ":musescore:",
  ["MuseScore Studio"] = ":musescore:",
  ["Music"] = ":music:",
  ["音乐"] = ":music:",
  ["Musique"] = ":music:",
  ["ミュージック"] = ":music:",
  ["Musik"] = ":music:",
  ["Chromatix"] = ":music:",
  ["Музика"] = ":music:",
  ["Музыка"] = ":music:",
  ["Native Access"] = ":native_instruments:",
  ["Navicat Premium"] = ":navicat:",
  ["Neovide"] = ":neovide:",
  ["neovide"] = ":neovide:",
  ["Neovim"] = ":neovim:",
  ["neovim"] = ":neovim:",
  ["nvim"] = ":neovim:",
  ["网易云音乐"] = ":netease_music:",
  ["NetEaseMusic"] = ":netease_music:",
  ["Netflix"] = ":netflix:",
  ["NetNewsWire"] = ":netnewswire:",
  ["MA 1 - Automatic Monitor Alignment"] = ":neumann:",
  ["News"] = ":news:",
  ["Nextcloud"] = ":nextcloud:",
  ["Nicotine+"] = ":nicotine_plus:",
  ["Nimble Commander"] = ":nimble_commander:",
  ["NimbleCommander-Unsigned"] = ":nimble_commander:",
  ["Noodl"] = ":noodl:",
  ["Noodl Editor"] = ":noodl:",
  ["NordVPN"] = ":nord_vpn:",
  ["Notability"] = ":notability:",
  ["Notes"] = ":notes:",
  ["备忘录"] = ":notes:",
  ["メモ"] = ":notes:",
  ["Notizen"] = ":notes:",
  ["Нотатки"] = ":notes:",
  ["Заметки"] = ":notes:",
  ["Notion"] = ":notion:",
  ["Notion Mail"] = ":notion_mail:",
  ["Nova"] = ":nova:",
  ["Numbers"] = ":numbers:",
  ["Numbers 表格"] = ":numbers:",
  ["nvALT"] = ":nvalt:",
  ["Nvidia GeForce Now"] = ":nvidia_geforce_now:",
  ["GeForceNOW"] = ":nvidia_geforce_now:",
  ["Nzbget"] = ":nzbget:",
  ["Obsidian"] = ":obsidian:",
  ["OBS"] = ":obsstudio:",
  ["Ollama"] = ":ollama:",
  ["OmniDiskSweeper"] = ":omni_disk_sweeper:",
  ["OmniFocus"] = ":omni_focus:",
  ["OmniRoute"] = ":omniroute:",
  ["1Password"] = ":one_password:",
  ["OneDrive"] = ":onedrive:",
  ["ONLYOFFICE"] = ":onlyoffice:",
  ["Open Video Downloader"] = ":open_video_downloader:",
  ["ChatGPT"] = ":openai:",
  ["ChatGPT Classic"] = ":openai:",
  ["OpenAI Translator"] = ":openai_translator:",
  ["OpenChamber"] = ":openchamber:",
  ["OpenClaw"] = ":openclaw:",
  ["opencode"] = ":opencode:",
  ["OpenCode"] = ":opencode:",
  ["OpenEmu"] = ":openemu:",
  ["OpenVPN Connect"] = ":openvpn_connect:",
  ["Opera"] = ":opera:",
  ["OrbStack"] = ":orbstack:",
  ["OrcaSlicer"] = ":orcaslicer:",
  ["Orion"] = ":orion:",
  ["Orion RC"] = ":orion:",
  ["Osaurus"] = ":osaurus:",
  ["Overcast"] = ":overcast:",
  ["Overleaf"] = ":overleaf:",
  ["ShareLaTeX"] = ":overleaf:",
  ["Overseer"] = ":overseer:",
  ["Seer"] = ":overseer:",
  ["Pages"] = ":pages:",
  ["Pages 文稿"] = ":pages:",
  ["Parallels Desktop"] = ":parallels:",
  ["Parcel"] = ":parcel:",
  ["Parsec"] = ":parsec:",
  ["Paseo"] = ":paseo:",
  ["Passepartout"] = ":passepartout:",
  ["Password Depot"] = ":password_depot:",
  ["AutoFillPanelService"] = ":passwords:",
  ["coreautha"] = ":passwords:",
  ["Passwords"] = ":passwords:",
  ["Passwörter"] = ":passwords:",
  ["Паролі"] = ":passwords:",
  ["Пароли"] = ":passwords:",
  ["PasteBar"] = ":pastebar:",
  ["PCSX2"] = ":pcsx2:",
  ["PDF Expert"] = ":pdf_expert:",
  ["Pearcleaner"] = ":pearcleaner:",
  ["Permute 3"] = ":permute:",
  ["Permute 4"] = ":permute:",
  ["Perplexity"] = ":perplexity:",
  ["Perplexity AI"] = ":perplexity:",
  ["pgAdmin 4"] = ":pgadmin:",
  ["Phoenix Slides"] = ":phoenix_slides:",
  ["Phone"] = ":phone:",
  ["Photos"] = ":photos:",
  ["Fotos"] = ":photos:",
  ["Фото"] = ":photos:",
  ["Adobe Photoshop"] = ":photoshop:",
  ["PhpStorm"] = ":php_storm:",
  ["Pi-hole Remote"] = ":pihole:",
  ["Pine"] = ":pine:",
  ["Pinta"] = ":pinta:",
  ["Pixcall"] = ":pixcall:",
  ["Pixelmator Pro"] = ":pixelmator_pro:",
  ["PixiEditor"] = ":pixieditor:",
  ["Play"] = ":play:",
  ["Plex"] = ":plex:",
  ["Plexamp"] = ":plexamp:",
  ["Plezy"] = ":plezy:",
  ["Podcasts"] = ":podcasts:",
  ["播客"] = ":podcasts:",
  ["Подкасти"] = ":podcasts:",
  ["Подкасты"] = ":podcasts:",
  ["PomoDone App"] = ":pomodone:",
  ["Postman"] = ":postman:",
  ["Premiere"] = ":premiere:",
  ["Adobe Premiere"] = ":premiere:",
  ["Adobe Premiere Pro 2024"] = ":premiere:",
  ["Preview"] = ":preview:",
  ["预览"] = ":preview:",
  ["Skim"] = ":preview:",
  ["zathura"] = ":preview:",
  ["Aperçu"] = ":preview:",
  ["プレビュー"] = ":preview:",
  ["Vorschau"] = ":preview:",
  ["Оглядач"] = ":preview:",
  ["Просмотр"] = ":preview:",
  ["Print Center"] = ":print_center:",
  ["Druckzentrale"] = ":print_center:",
  ["Pro Tools"] = ":pro_tools:",
  ["Problem Reporter"] = ":problem_reporter:",
  ["Proton Mail"] = ":proton_mail:",
  ["Proton Mail Bridge"] = ":proton_mail:",
  ["Proton VPN"] = ":proton_vpn:",
  ["ProtonVPN"] = ":proton_vpn:",
  ["Prowlarr"] = ":prowlarr:",
  ["Proxyman"] = ":proxyman:",
  ["PrusaSlicer"] = ":prusaslicer:",
  ["SuperSlicer"] = ":prusaslicer:",
  ["PS Remote Play"] = ":ps_remote_play:",
  ["PyCharm"] = ":pycharm:",
  ["qBittorrent"] = ":qbittorrent:",
  ["QBZ"] = ":qbz:",
  ["QGIS"] = ":qgis:",
  ["QLMarkdown"] = ":qlmarkdown:",
  ["Qobuz"] = ":qobuz:",
  ["QQ"] = ":qq:",
  ["QQ音乐"] = ":qqmusic:",
  ["QQMusic"] = ":qqmusic:",
  ["Quantumult X"] = ":quantumult_x:",
  ["Quark"] = ":quark:",
  ["夸克"] = ":quark:",
  ["QuickTime Player"] = ":quicktime:",
  ["Quip"] = ":quip:",
  ["qutebrowser"] = ":qute_browser:",
  ["Radarr"] = ":radarr:",
  ["Raindrop.io"] = ":raindrop_io:",
  ["Raspberry Pi Imager"] = ":raspberry_pi:",
  ["Raspberry Pi Connect"] = ":raspberry_pi:",
  ["Raycast"] = ":raycast:",
  ["Raycast Beta"] = ":raycast:",
  ["rclone"] = ":rclone:",
  ["Rclone"] = ":rclone:",
  ["Rclone UI"] = ":rclone:",
  ["REAPER"] = ":reaper:",
  ["Reeder"] = ":reeder5:",
  ["rekordbox"] = ":rekordbox:",
  ["Remind Faster"] = ":remind_me_faster:",
  ["Reminders"] = ":reminders:",
  ["提醒事项"] = ":reminders:",
  ["Rappels"] = ":reminders:",
  ["リマインダー"] = ":reminders:",
  ["Erinnerungen"] = ":reminders:",
  ["Нагадування"] = ":reminders:",
  ["Напоминания"] = ":reminders:",
  ["RenameMyTVSeries"] = ":rename_my_tv_series:",
  ["Rename My TV Series"] = ":rename_my_tv_series:",
  ["Replit"] = ":replit:",
  ["Repo Prompt"] = ":repo_prompt:",
  ["Resilio Sync"] = ":resilio_sync:",
  ["Rider"] = ":rider:",
  ["JetBrains Rider"] = ":rider:",
  ["Rio"] = ":rio:",
  ["Roblox"] = ":roblox:",
  ["Royal TSX"] = ":royaltsx:",
  ["RStudio"] = ":rstudio:",
  ["RsyncUI"] = ":rsyncui:",
  ["ruffle"] = ":ruffle:",
  ["Ruffle"] = ":ruffle:",
  ["RustDesk"] = ":rustdesk:",
  ["SABnzbd"] = ":sabnzbd:",
  ["Safari"] = ":safari:",
  ["Safari浏览器"] = ":safari:",
  ["Safari Technology Preview"] = ":safari:",
  ["Screencap"] = ":screencap:",
  ["Scribus"] = ":scribus:",
  ["Script Debugger"] = ":script_debugger:",
  ["Script Editor"] = ":script_editor:",
  ["Seafile"] = ":seafile:",
  ["Secretive"] = ":secretive:",
  ["Secretive.app"] = ":secretive:",
  ["Sequel Ace"] = ":sequel_ace:",
  ["Sequel Pro"] = ":sequel_pro:",
  ["Session"] = ":session:",
  ["Setapp"] = ":setapp:",
  ["SF Symbols"] = ":sf_symbols:",
  ["SF Symbole"] = ":sf_symbols:",
  ["SF-Symbole"] = ":sf_symbols:",
  ["SF Symbols Beta"] = ":sf_symbols:",
  ["Shapr3D"] = ":shapr3d:",
  ["Shortcuts"] = ":shortcuts:",
  ["Команди"] = ":shortcuts:",
  ["Команды"] = ":shortcuts:",
  ["Sibelius"] = ":sibelius:",
  ["Signal"] = ":signal:",
  ["sioyek"] = ":sioyek:",
  ["Siri"] = ":siri:",
  ["Sketch"] = ":sketch:",
  ["Skype"] = ":skype:",
  ["Slack"] = ":slack:",
  ["SmartGit"] = ":smartgit:",
  ["SnapGene"] = ":snapgene:",
  ["SnippetsLab"] = ":snippetslab:",
  ["solidtime"] = ":solidtime:",
  ["Sonarr"] = ":sonarr:",
  ["SourceGit"] = ":sourcegit:",
  ["Sourcetree"] = ":sourcetree:",
  ["Spark Desktop"] = ":spark:",
  ["Sparkle"] = ":sparkle:",
  ["Spotify"] = ":spotify:",
  ["Spotlight"] = ":spotlight:",
  ["StarUML"] = ":staruml:",
  ["Steam"] = ":steam:",
  ["Steam Helper"] = ":steam:",
  ["Stickies"] = ":stickies:",
  ["Stocks"] = ":stocks:",
  ["Stremio"] = ":stremio:",
  ["Studio 3T"] = ":studio_3t:",
  ["Sublime Merge"] = ":sublime_merge:",
  ["Sublime Text"] = ":sublime_text:",
  ["Summoners War"] = ":summoners_war:",
  ["Super Productivity"] = ":super_productivity:",
  ["superProductivity"] = ":super_productivity:",
  ["Surfshark"] = ":surfshark:",
  ["Swift Playground"] = ":swift_playground:",
  ["Synergy"] = ":synergy:",
  ["System Information"] = ":system_information:",
  ["System Profiler"] = ":system_information:",
  ["T3 Code"] = ":t3_code:",
  ["T3 Code (Alpha)"] = ":t3_code:",
  ["T3 Chat"] = ":t3chat:",
  ["Tabby"] = ":tabby:",
  ["TablePlus"] = ":tableplus:",
  ["Tailscale"] = ":tailscale:",
  ["Tana"] = ":tana:",
  ["Tautulli"] = ":tautulli:",
  ["TeamSpeak 3"] = ":team_speak:",
  ["TeamViewer"] = ":teamviewer:",
  ["Telegram"] = ":telegram:",
  ["Terminal"] = ":terminal:",
  ["终端"] = ":terminal:",
  ["ターミナル"] = ":terminal:",
  ["Термінал"] = ":terminal:",
  ["Терминал"] = ":terminal:",
  ["Termius"] = ":termius:",
  ["TestFlight"] = ":testflight:",
  ["Typora"] = ":text:",
  ["TextEdit"] = ":textedit:",
  ["The Sims 3"] = ":the_sims:",
  ["The Sims 4"] = ":the_sims:",
  ["Microsoft To Do"] = ":things:",
  ["Things"] = ":things:",
  ["Thunderbird"] = ":thunderbird:",
  ["Thunderbird Daily"] = ":thunderbird:",
  ["TI Connect CE"] = ":ti_connect_ce:",
  ["TickTick"] = ":tick_tick:",
  ["TIDAL"] = ":tidal:",
  ["TigerVNC"] = ":tigervnc:",
  ["Time Machine"] = ":time_machine:",
  ["TimeMachineEditor"] = ":time_machine:",
  ["TheTimeMachineMechanic"] = ":time_machine:",
  ["Timery"] = ":timery:",
  ["Timing"] = ":timingapp:",
  ["Tiny RDM"] = ":tinyrdm:",
  ["Tips"] = ":tips:",
  ["Todoist"] = ":todoist:",
  ["Toggl Track"] = ":toggl_track:",
  ["Tor Browser"] = ":tor_browser:",
  ["Tot"] = ":tot:",
  ["Tower"] = ":tower:",
  ["TradingView"] = ":trading_view:",
  ["Transmission"] = ":transmission:",
  ["Transmit"] = ":transmit:",
  ["Trello"] = ":trello:",
  ["Tuple"] = ":tuple:",
  ["Twingate"] = ":twingate:",
  ["Tweetbot"] = ":twitter:",
  ["Twitter"] = ":twitter:",
  ["UGREEN NAS"] = ":ugreen-nas:",
  ["Unity"] = ":unity:",
  ["Unity Hub"] = ":unity:",
  ["Usenapp"] = ":usenapp:",
  ["UTM"] = ":utm:",
  ["VeraCrypt"] = ":veracrypt:",
  ["Vesktop"] = ":vesktop:",
  ["Viber"] = ":viber:",
  ["MacVim"] = ":vim:",
  ["Vim"] = ":vim:",
  ["VimR"] = ":vim:",
  ["VirtualBox"] = ":virtualbox:",
  ["Visual Studio"] = ":visual_studio:",
  ["Vivaldi"] = ":vivaldi:",
  ["VLC"] = ":vlc:",
  ["VMware Fusion"] = ":vmware_fusion:",
  ["Voice Memos"] = ":voice_memos:",
  ["Vorta"] = ":vorta:",
  ["VSCodium"] = ":vscodium:",
  ["Warp"] = ":warp:",
  ["Wave"] = ":waveterm:",
  ["Waydir"] = ":waydir:",
  ["Weather"] = ":weather:",
  ["Wetter"] = ":weather:",
  ["Погода"] = ":weather:",
  ["WebStorm"] = ":web_storm:",
  ["WebTorrent"] = ":webtorrent:",
  ["Webull Desktop"] = ":webull:",
  ["微信"] = ":wechat:",
  ["WeChat"] = ":wechat:",
  ["企业微信"] = ":wecom:",
  ["WeCom"] = ":wecom:",
  ["WeTransfer"] = ":wetransfer:",
  ["WezTerm"] = ":wezterm:",
  ["wezterm-gui"] = ":wezterm:",
  ["WhatCable"] = ":whatcable:",
  ["WhatsApp"] = ":whats_app:",
  ["‎WhatsApp"] = ":whats_app:",
  ["Windows App"] = ":windows_app:",
  ["WireGuard"] = ":wireguard:",
  ["Wireshark"] = ":wireshark:",
  ["Wispr Flow"] = ":wispr_flow:",
  ["xemu"] = ":xbox:",
  ["Xcode"] = ":xcode:",
  ["XMind"] = ":xmind:",
  ["Yaak"] = ":yaak:",
  ["Yandex Browser"] = ":yandex_browser:",
  ["Yandex Browser"] = ":yandex_browser:",
  ["Yandex"] = ":yandex_browser:",
  ["Yandex Music"] = ":yandex_music:",
  ["Yazi"] = ":yazi:",
  ["yazi"] = ":yazi:",
  ["YouTube"] = ":youtube:",
  ["YouTube Music"] = ":youtube_music:",
  ["Yubico Authenticator"] = ":yubico:",
  ["YubiKey Manager"] = ":yubico:",
  ["Yuque"] = ":yuque:",
  ["语雀"] = ":yuque:",
  ["Z-Library"] = ":z_library:",
  ["Zed"] = ":zed:",
  ["Zed Preview"] = ":zed:",
  ["Zen"] = ":zen_browser:",
  ["Zen Browser"] = ":zen_browser:",
  ["Twilight"] = ":zen_browser:",
  ["Zeplin"] = ":zeplin:",
  ["zoom.us"] = ":zoom:",
  ["Zoom"] = ":zoom:",
  ["Zotero"] = ":zotero:",
  ["Zulip"] = ":zulip:",

  -- Traductions françaises absentes de la table amont. Les noms sont ceux
  -- qu'affiche macOS et que transmet AeroSpace, en forme composée (NFC) :
  -- la forme décomposée que renvoie mdls ne correspondrait pas.

  ["Moniteur d’activité"] = ":activity_monitor:", -- Activity Monitor
  ["Utilitaire AirPort"] = ":airport_utility:", -- AirPort Utility
  ["Configuration audio et MIDI"] = ":audio_midi_setup:", -- Audio MIDI Setup
  ["Échange de fichiers Bluetooth"] = ":bluetooth:", -- Bluetooth File Exchange
  ["Livres"] = ":apple_books:", -- Books
  ["Calculatrice"] = ":calculator:", -- Calculator
  ["Échecs"] = ":chess:", -- Chess
  ["Horloge"] = ":clock:", -- Clock
  ["Dictionnaire"] = ":dictionary:", -- Dictionary
  ["Colorimètre numérique"] = ":color_picker:", -- Digital Color Meter
  ["Utilitaire de disque"] = ":disk_utility:", -- Disk Utility
  ["Livre des polices"] = ":font_book:", -- Font Book
  ["Jeux"] = ":games:", -- Games
  ["Maison"] = ":home:", -- Home
  ["Loupe"] = ":magnifier:", -- Magnifier
  ["Plans"] = ":maps:", -- Maps
  ["Mots de passe"] = ":passwords:", -- Passwords
  ["Téléphone"] = ":phone:", -- Phone
  ["Centre d’impression"] = ":print_center:", -- Print Center
  ["Symboles SF"] = ":sf_symbols:", -- SF Symbols
  ["Éditeur de script"] = ":script_editor:", -- Script Editor
  ["Raccourcis"] = ":shortcuts:", -- Shortcuts
  ["Aide-mémoire"] = ":stickies:", -- Stickies
  ["Bourse"] = ":stocks:", -- Stocks
  ["Informations système"] = ":system_information:", -- System Information
  ["Astuces"] = ":tips:", -- Tips
  ["Visual Studio Code"] = ":code:", -- Code
  ["Dictaphone"] = ":voice_memos:", -- Voice Memos
  ["Localiser"] = ":find_my:", -- Find My
  ["Météo"] = ":weather:", -- Weather
  ["Recopie de l’iPhone"] = ":iphone_mirroring:", -- iPhone Mirroring
}

return function(app_name)
  -- Correspondance exacte d'abord
  if icon_map_table[app_name] then
    return icon_map_table[app_name]
  end

  -- Applications dont le nom porte un numéro de version
  if app_name:match("^Adobe Bridge") then return ":adobe_bridge:" end
  if app_name:match("^Adobe Illustrator") then return ":illustrator:" end
  if app_name:match("^Adobe InDesign") then return ":indesign:" end
  if app_name:match("^Adobe Photoshop") then return ":photoshop:" end
  if app_name:match("^MongoDB Compass") then return ":mongodb:" end

  return ":default:"
end
